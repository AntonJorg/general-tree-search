...
class Control:
    ...
    def if_depth_reached_or_fully_expanded(
        self, node: TreeSearchNode, value: float = 0
    ) -> bool:
        return (
    ...
    def timed_termination(self):
        if self.search_time is None:
            raise AttributeError(
                "Subclass must set self.search_time to use self.timed_termination"
            )

        return (
            time.process_time_ns() - self.start_time > self.search_time * 1_000_000_000
        )
