...
if __name__ == "__main__":
    agent_dict = {agent.name: agent for agent in predefined}
    state_dict = {state.__name__.replace("State", ""): state for state in adversarial}
    ...
    def run_game(i, agent0, agent1, agent0_args, agent1_args, swap):
        ...
        agents = (
            agent_dict[agent0].to_builder().add_params(search_time=args.t).build(agent0),
            agent_dict[agent1].to_builder().add_params(search_time=args.t).build(agent1)
        )
